...
function main()
	...
	-- COMMANDS
	sampRegisterChatCommand("logo", function(cmd, num) -- HELP
		...
		elseif #cmd == 0 then -- OPEN HELP MENU
			...
			if cmd:find("^type") then -- ERROR TYPE
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
				sampAddChatMessage(TAG .."{FFFFFF}Неизвестная команда. Введите: {FFCC00}" .. CMD_HELP .. " type (1-4)", COLOR_CHAT["ERROR"])
			else -- ERROR CMD
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
...
{AFE7FF}/bars (up)date{ffffff} - Автообновление (Включить/Выключить)
...
{AFE7FF}Дата последнего обновления{ffffff}		10.06.2022
]]
