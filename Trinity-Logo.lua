...
local config = { 
	["rpg"] = {style = 1, posX = 100, posY = 100, view = true, view_standart = true, color = "FFFFFF", newyearbg = true},
	["rp1"] = {style = 1, posX = 100, posY = 100, view = true, view_standart = true, color = "FFFFFF", newyearbg = true},
	["rp2"] = {style = 1, posX = 100, posY = 100, view = true, view_standart = true, color = "FFFFFF", newyearbg = true},
	update = false
}
		...
		imgui.BeginChild(u8 "##block_1", imgui.ImVec2(300, 520), true, imgui.WindowFlags.AlwaysUseWindowPadding)
			...
			-- local isServer = "rp1"
				...
				if data[isServer].style == 2 then
					if imgui.ColorEdit3(u8"Изменить цвет логотипа", color, imgui.ColorEditFlags.NoOptions + imgui.ColorEditFlags.NoInputs) then
						local colorARGB = join_argb(0, color.v[1]* 255, color.v[2]* 255, color.v[3]* 255)
						data[isServer].color = ("%06X"):format(colorARGB)
						isSaveChanges = true
					end
				else
...
function createLogo()
	if not data[isServer].view or isKeyDown(0x77) then
		if data[isServer].view_standart then
	...
	if data[isServer].view then
		if standartLogoHide then
			...
			elseif data[isServer].style == 2 then
				renderDrawTexture(load_texture["logo-color"], posX, posY, sizeX, sizeY, 0, "0xFF" .. data[isServer].color) -- Logo Colored
