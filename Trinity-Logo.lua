...
function main()
	...
	sampRegisterChatCommand("logo", function(cmd) -- HELP
		...
		elseif (cmd == "resetall") then -- REMOVE CACHE
			removeFilesCache()
			addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			sampAddChatMessage(string.format("%s{FFFFFF}Вы произвели {FF3232}сброс всех{FFFFFF} файлов ресурса {FFCC00}%s.", TAG, thisScript().name), COLOR_CHAT["DONE"])
			reloadScript()
...
function createLogo()
	-- DELETE STANDART LOGO
	if sampTextdrawIsExists(standartLogoID) then
		sampTextdrawDelete(standartLogoID)
	end
...
{AFE7FF}/logo (up)date{ffffff} - Автообновление (Включить/Выключить)

{ffcc00}Полезные советы:{ffffff}

{AFE7FF}[1]{ffffff} При замене лого на свое, используйте размер {ffcc00}256x64{ffffff} пикселя.
